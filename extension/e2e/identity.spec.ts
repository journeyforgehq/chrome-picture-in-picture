...
test("fresh install generates a device id and caches the free tier", async ({ ext }) => {
  ...
  await page.goto(`chrome-extension://${ext.extensionId}/options.html`);

  // The options page renders "free" by default before init finishes, so poll
  // storage for the device-id write rather than gating on visible text (see
  // waitForDeviceId).
